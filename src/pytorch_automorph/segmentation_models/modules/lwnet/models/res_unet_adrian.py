...
class ConvBlock(torch.nn.Module):
    def __init__(self, in_c, out_c, k_sz=3, shortcut=False, pool=True):
        ...
        conv1 = nn.Conv2d(in_c, out_c, kernel_size=k_sz, padding=pad)
        block.append(conv1)
        block.append(nn.ReLU())
        block.append(nn.BatchNorm2d(out_c))

        conv2 = nn.Conv2d(out_c, out_c, kernel_size=k_sz, padding=pad)
        block.append(conv2)
        block.append(nn.ReLU())
        block.append(nn.BatchNorm2d(out_c))
